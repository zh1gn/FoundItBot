...
def _build_items_menu(items, user_id, bot_username):
    """Build text + keyboard for the My Items screen."""
    from config.config import BOT_USERNAME as _BU
    _bu = bot_username or _BU
    ...
    if not items:
        text = (
            ...
            "💡 Чем больше вещей вы зарегистрируете, тем выше шанс их найти!"
        )
        keyboard = [[InlineKeyboardButton("➕ Добавить вещь", callback_data='add_item')]]
        return text, InlineKeyboardMarkup(keyboard)

    text = f"📋 Мои вещи ({len(items)})\n"
    text += "─" * 28 + "\n\n"

    for i, item in enumerate(items, 1):
        emoji = ITEM_TYPES.get(item['item_type'], '📦')
        qr_url = f"https://t.me/{_bu}?start=found_{item['qr_id']}"
        found_line = f"\n   🔍 Найдена {item['times_found']} раз(а)" if item['times_found'] > 0 else ""
        text += (
            f"{i}. {emoji} {item['name']}\n"
            f"   QR: {item['qr_id']}{found_line}\n"
            f"   Тип: {item['item_type']}  |  Добавлена: {item['added_at'][:10]}\n"
            f"   Ссылка для QR-кода:\n"
            f"   {qr_url}\n\n"
        )

    # One button per item for quick QR link copy + bulk actions
    keyboard = []
    for item in items:
        emoji = ITEM_TYPES.get(item['item_type'], '📦')
        qr_url = f"https://t.me/{_bu}?start=found_{item['qr_id']}"
        keyboard.append([InlineKeyboardButton(
            f"{emoji} {item['name']} — скопировать ссылку QR",
            callback_data=f"item_qr:{item['qr_id']}"
        )])

    keyboard.append([
        InlineKeyboardButton("➕ Добавить вещь", callback_data='add_item'),
        InlineKeyboardButton("🗑️ Удалить", callback_data='delete_item'),
    ])
...
async def myitems_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text, reply_markup = _build_items_menu(items, user_id, BOT_USERNAME)
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if success:
        ...
        text = (
            f"✅ Вещь успешно добавлена!\n\n"
            f"{emoji} {item_name}\n"
            f"QR-код: {qr_id}\n"
            f"Тип: {item_type}\n\n"
            f"── Следующие шаги ──\n\n"
            f"Онлайн генератор QR-кода:\n"
            f"1. Откройте qr-code-generator.com\n"
            f"2. Выберите тип URL\n"
            f"3. Вставьте эту ссылку:\n"
            f"{qr_url}\n"
            f"4. Скачайте и распечатайте QR-код\n"
            f"5. Наклейте на вещь\n\n"
            f"💡 При сканировании нашедший сразу свяжется с вами!\n"
            f"🔒 Ваш номер телефона будет скрыт"
        )
        ...
        if new_achievements:
            ...
            if 'achievement_msg_ids' not in context.user_data:
                context.user_data['achievement_msg_ids'] = []
            for achievement in new_achievements:
                achievement_text = get_achievement_message(achievement)
                sent = await update.message.reply_text(achievement_text)
                context.user_data['achievement_msg_ids'].append(sent.message_id)
...
async def found_handler(update: Update, context: ContextTypes.DEFAULT_TYPE, qr_id: str):
    ...
    emoji = ITEM_TYPES.get(item['item_type'], '📦')
    finder_username = update.effective_user.username  # может быть None

    # Сообщение нашедшему
    await update.message.reply_text(
        f"✅ Спасибо за честность!\n\n"
        f"Вы нашли: {emoji} {item['name']}\n\n"
        f"Владелец получил уведомление и свяжется с вами в ближайшее время! 🤝\n\n"
        f"💡 Оставайтесь на связи в Telegram."
    )

    # Сообщение владельцу — кнопка работает и на ПК и на телефоне
    try:
        # Если есть username — используем https://t.me/username (работает везде)
        # Если нет — используем tg://user?id= (только мобильный) + показываем имя
        if finder_username:
        ...
        keyboard = [[InlineKeyboardButton(contact_label, url=contact_url)]]
        reply_markup = InlineKeyboardMarkup(keyboard)

        owner_text = (
            f"🎉 ВАША ВЕЩЬ НАЙДЕНА!\n\n"
            f"{emoji} Вещь: {item['name']}\n"
            f"👤 Нашёл: {finder_name}"
            + (f" (@{finder_username})" if finder_username else "") +
            f"\n⏰ Время: {datetime.now().strftime('%H:%M, %d.%m.%Y')}\n"
            f"🆔 QR-код: {qr_id}\n\n"
            f"Нажмите кнопку ниже чтобы написать нашедшему!\n"
            f"💡 Не забудьте поблагодарить за честность! 🙏"
        )
        if not finder_username:
            owner_text += (
                f"\n\n⚠️ У нашедшего нет username. Если кнопка не работает на ПК — "
                f"найдите его вручную по имени: {finder_name}"
            )
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Удаляем сообщения о достижениях при любом нажатии кнопки
    for msg_id in context.user_data.pop('achievement_msg_ids', []):
        try:
...
reply_markup=reply_markup
    ...
    elif query.data.startswith('item_qr:'):
        # Показываем детальную информацию о вещи + QR ссылку
        qr_id = query.data.split(':', 1)[1]
        item = db.get_item_by_qr(qr_id)
        if not item:
        ...
        text = (
            ...
            f"Добавлена: {item['added_at'][:10]}{found_info}\n\n"
            f"Ссылка для генерации QR-кода:\n"
            f"{qr_url}\n\n"
            f"Скопируйте ссылку и вставьте на сайте qr-code-generator.com"
        )
        keyboard = [
            [InlineKeyboardButton("🌐 Создать QR онлайн", url="https://qr-code-generator.com")],
            [InlineKeyboardButton("🗑️ Удалить эту вещь", callback_data=f"confirm_delete:{qr_id}")],
            [InlineKeyboardButton("◀️ Назад к списку", callback_data='my_items')],
        ]
        try:
            await query.message.edit_text(text, reply_markup=InlineKeyboardMarkup(keyboard), disable_web_page_preview=True)
        except Exception:
    ...
    elif query.data.startswith('do_delete:'):
        qr_id = query.data.split(':', 1)[1]
        success = db.delete_item(qr_id, user_id)
        if success:
            await query.answer(f"✅ {qr_id} удалён", show_alert=False)
        else:
